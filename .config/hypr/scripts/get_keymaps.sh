...
jq -r '
  ...
  [.[] | select(.has_description and .description != "") | { keys: keys, description, submap }]
  | (map(.keys | length) | max + 2) as $width
  | .[]
  | .keys + (" " * ($width - (.keys | length))) + .description + (" " * ($width - (.keys | length))) + .submap
' | walker -d -H -p 'Search keybindings' --width 950 --height 600
